package pro.chukai.search.core;

import pro.chukai.search.config.DirectoryType;
import pro.chukai.search.config.SearchConfig;
...
import static pro.chukai.search.core.SearchDocument.CONTEXT;
import static pro.chukai.search.core.SearchDocument.ID;
import static pro.chukai.search.core.SearchDocument.TITLE;

/**
 * 创建索引及查询操作
 * Created by chukai on 2016/11/7.
 */
...
public class SearchImpl implements Search, InitializingBean, DisposableBean {
    ...
    public List<SearchDocument> search(String queryStr, int num) throws Exception {
        ...
        for (ScoreDoc scoreDoc : topDocs.scoreDocs) {
            ...
            result.add(new SearchDocument(doc.get(ID), highTitle == null ? title : highTitle, highContext == null ? subContext(content) : highContext));
        }
        return result;
    }
